...
func TestNewServer_ValidAddress(t *testing.T) {
	s, err := NewServer(ServerConfig{Address: "localhost:7400"})
	if err != nil {
...
func TestServer_Start_NotImplemented(t *testing.T) {
	s, _ := NewServer(ServerConfig{Address: "localhost:7400"})
...
func TestNewClient_ValidServer(t *testing.T) {
	c, err := NewClient(ClientConfig{Server: "localhost:7400"})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if c == nil {
...
func TestClient_Connect_NotImplemented(t *testing.T) {
	c, _ := NewClient(ClientConfig{Server: "localhost:7400"})
