...
**这不是协议栈 bug**，是测试代码的 ctx 生命周期错误。但它暴露了一个**产品 API 设计缺陷**：

> 原实现 `Client.Connect(ctx)` 用传入的 `ctx` 派生 `lifecycleCtx`（驱动 readControlLoop/ackReaper/heartbeat）。调用方必须传入长生命周期 ctx，否则后台 goroutine 会随 ctx 取消而退出——这是一个反直觉的陷阱。

### 彻底修复（产品代码）

参照 Go 惯例（`grpc.DialContext`、`http.Server.ListenAndServe`）：**ctx 只控制握手阶段，client 生命周期由 `Close()` 独立控制**。

`v1/client.go` `Connect`：
```go
// 修复前（陷阱）：
...
- caller 传入的 `ctx` 仅用于握手超时（Dial、OpenStream、HELLO/AUTH）；
- 握手成功后，后台 goroutine 运行在独立的 `lifecycleCtx` 上，不随 `ctx` 取消退出；
- client 生命周期完全由 `Close()` 终结（`Close` 内部调 `lifecycleCancel`）。
