...
### 6.2.2 W3C Trace Context 协议级承载（capability `w3c_trace_context`）

当外部请求（被 APM/上游服务监控）携带 W3C `traceparent: 00-<trace-id>-<span-id>-<flags>` 与 `tracestate` 进入 MBTA 边界时，需要协议级注入点把这些上下文关联起来，否则只能逐 signal 塞 `attributes`——丢失采样位（flags）、tracestate 键值对与跨协议边界的一致性语义，每个 MBTA 消费者各自打补丁。`w3c_trace_context` 提供这一契约：

- **per-signal 承载**：`SignalRecord.trace_flags`（W3C trace-flags，低 8 位有效，`> 0xff` 拒绝）+ `SignalRecord.trace_state`（W3C tracestate，**有序**键值对列表，≤ 32 成员，不可用 map）。
- **batch/stream 级继承头**：`BatchMessage.trace_context`（`TraceContext{trace_id, span_id, parent_span_id, trace_flags, trace_state}`）。同一 trace 的多个 signal 共享一份上下文，signal 偏离时才单独写——与逐信号冗余优化合并（§6.2 缺口 2）。
- **协商**：HELLO 阶段双方均宣告 `w3c_trace_context` 才启用 batch 级继承头；per-signal 字段常驻（与 `trace_id` 同级），不依赖该 capability。
- **格式约束**：`trace_context` 内的 `trace_id`/`span_id`/`parent_span_id` 复用 §6.2.1 的 W3C hex 格式约束；tracestate 的 key/value 不含控制字符、长度受 `validateTextField` 约束。

未声明该 capability 时，traceparent 语义无协议级承载点（退化为历史行为）。
...
| `w3c_trace_context`                         | stable | batch/stream 级 W3C traceparent/tracestate 协议级承载 | §6.2.2 |
